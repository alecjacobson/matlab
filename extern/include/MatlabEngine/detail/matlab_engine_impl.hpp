        ...
        namespace {
            ...
            FutureResult<void> setPropertyAsyncImpl(matlab::data::Array &objectArray, size_t index, const std::u16string &propertyName, const matlab::data::Array &property, matlab::engine::MATLABEngine* eng, uint64_t engHandle, bool isScalar) {
                ...
                if (objectArray.getType() == matlab::data::ArrayType::VALUE_OBJECT) {
                    ...
                    std::promise<void>* p = new std::promise<void>();
                    std::future<void> f = p->get_future();
                    uintptr_t handle = engine_feval_with_completion(engHandle, "matlab.internal.engine.setProperty", 0, true, argsImpl, nrhs, &set_feval_promise_data, &set_feval_promise_exception, p, nullptr, nullptr, &writeToStreamBuffer, &deleteStreamBufferImpl);

                    ret = FutureResult<void>(std::move(f), std::make_shared<TaskReference>(handle, &engine_cancel_feval_with_completion));
                }
                else {
